...
### TL;DR;
  ...
  * private key is just a number (like a PIN code) but a very large one, order of magnitude is about `10^77` (put this in context of number of atoms in the known, observable universe which is `10^80`)
...
We have 6 domain parameters as defined in [secp256k1](https://en.bitcoin.it/wiki/Secp256k1): p,a,b,G,n,h
...
We all know that Bitcoin's private key is 256 bits long that has be an integer between `0` and `n` the order of subgroup. Here is my take defined as binary string.
